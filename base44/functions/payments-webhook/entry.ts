...
async function handleOrderApproved(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // WaterRest grants the whole app: flip the buyer's UserProfile.subscription_status to "active".
  // Uses the logged-in buyer's appUserId when known; otherwise matches the checkout email to a User.
  // The update is naturally idempotent, so duplicate ORDER_APPROVED deliveries are safe.
  let grantUserId = purchase.appUserId ?? null;
  if (!grantUserId && buyerEmail) {
    try {
      const matched = await db.entities.User.filter({ email: buyerEmail });
      grantUserId = matched?.[0]?.id ?? null;
    } catch (e) {
      console.error("payments-webhook: user lookup by email failed", e);
    }
  }
  if (grantUserId) {
    try {
      const profiles = await db.entities.UserProfile.filter({ created_by_id: grantUserId });
      const up = profiles?.[0];
      if (up) {
        await db.entities.UserProfile.update(up.id, { subscription_status: "active" });
      } else {
        console.warn("payments-webhook: no UserProfile found to grant", { grantUserId, checkoutId });
      }
    } catch (e) {
      console.error("payments-webhook: granting subscription failed", e);
      throw e; // stay "pending" so Wix retries the grant
    }
  } else {
    console.warn("payments-webhook: cannot grant — no appUserId or matching buyer email", { checkoutId });
  }
  // ===== END APP-SPECIFIC =====
...
async function handleSubscriptionEnded(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Revoke access: flip the buyer's UserProfile.subscription_status back to "canceled".
  // Mirrors the grant's user resolution and is idempotent.
  let revokeUserId = purchase.appUserId ?? null;
  if (!revokeUserId && purchase.buyerEmail) {
    try {
      const matched = await db.entities.User.filter({ email: purchase.buyerEmail });
      revokeUserId = matched?.[0]?.id ?? null;
    } catch (e) {
      console.error("payments-webhook: user lookup by email failed (revoke)", e);
    }
  }
  if (revokeUserId) {
    try {
      const profiles = await db.entities.UserProfile.filter({ created_by_id: revokeUserId });
      const up = profiles?.[0];
      if (up) {
        await db.entities.UserProfile.update(up.id, { subscription_status: "canceled" });
      }
    } catch (e) {
      console.error("payments-webhook: revoking subscription failed", e);
      throw e; // stay as-is so Wix retries the revoke
    }
  }
  // ===== END APP-SPECIFIC =====
