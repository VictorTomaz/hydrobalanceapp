...
Deno.serve(async (req: Request) => {
  try {
    ...
    // ===== APP-SPECIFIC =====
    // WaterRest sells a single monthly subscription. Price is resolved SERVER-SIDE — the client
    // sends only a product identifier; the authoritative price lives in this map, not the request.
    const productId = String(body.productId ?? "");
    const PRODUCTS = {
      "waterrest_pro_monthly": {
        name: "WaterRest Pro — Monthly",
        price: "1.99",
        currency: "USD",
        subscriptionInfo: {
          subscriptionSettings: { frequency: "MONTH" }, // infinite auto-renewing monthly
          title: "WaterRest Pro Monthly",
          description: "Unlimited access to all WaterRest hydration and wind-down features, billed monthly.",
        },
      },
    };
    ...
    // A subscription is a fixed-entitlement plan — hard-code quantity 1 and ignore the body.
    const quantity = 1;
